...
import { editProjectTemp, updateProject, imageUpload, checkEditTemplateDateTime } from '../../utils/Api';
...
const EditProject = () => {
  ...
  React.useEffect(() => {
    ...
    editProjectTemp({ project_id: params.id }).then((res) => {
      // console.log(res, 'res+++++Edit Project  ')
      if (res.code == 200) {
        let data = res.data
        data.templates.map((element, i) => {
          element['start_time_err'] = false
          element['end_time_err'] = false
          element['item_speed_err'] = false
          element['start_time_e'] = element.created_at.split('T')[0] + ' ' + element.start_time
          element['end_time_e'] = element.created_at.split('T')[0] + ' ' + element.end_time
          element.data_items.map((item, i)=>{
            item['item_name_err'] = false
            item['lamp_err'] = false
            item['picture_err'] = false
            item['video_err'] = false
          })
        })
        // data.project_date = data.project_date.split('T')[0] //Modify Date
  ...
  const checkValidations = (data) => {
    ...
    for (let i = 0; i <= data.templates.length - 1; i++) {
      ...
      if (data.templates[i].start_time == '' || data.templates[i].start_time_e == null) {
      ...
      if (data.templates[i].end_time == '' || data.templates[i].end_time_e == null) {
      ...
      for (let j = 0; j <= data.templates[i].data_items.length - 1; j++) {
        // console.log('data',data.templates[i].data_items)
            //Temp Value 1
  ...
  const saveTemplate1 = (index) => {
    ...
    //     NotificationManager.error('Please enter unique Lamp Number')

    // } else {
    //     NotificationManager.success('Template Add SuccessFully')
  ...
  const updateProj = (e) => {
    e.preventDefault();
    // console.log('check gain tempform', tempform)
    ...
    checkEditTemplateDateTime(tempform).then((res) => {
  ...
  const generateButton = (e) => {
    ...
    // console.log('validation_check', validate)
    if (validate != true) {
      updateProject(tempform).then((res) => {
        // console.log('ress', res)
        if (res.code === 200) {
          NotificationManager.success(data.project_update_msg)
          navigate('/project')
        } else {
          NotificationManager.error(data.project_err_msg)
        }
...
<Container>
  <Grid style={{ textAlign: 'center' }} py={3}>
    <h2 className='create_project'>{data.update_project_title}</h2>
  </Grid>
    ...
    <Grid style={{ textAlign: 'center', display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }} item xs={12}>
      ...
      <Grid style={{ margin: '0 10px' }} item xs={12} sm={6} md={6} >
        <TextField
          ...
          margin="normal"
          name='project_name'
          value={tempform?.project_name ? tempform?.project_name : ''}
        ...
        {/* <LocalizationProvider dateAdapter={AdapterDayjs}>
          ...
          <DatePicker
            ...
            value={tempform?.project_date ? tempform?.project_date : date}
            onChange={handleDate}
      ...
      <Grid style={{ margin: '0 10px' }} item xs={12} sm={6} md={10}>
                        ...
                        <Grid className={item?.item_speed_err ? 'item_speed_default_err' : 'item_speed_default'}>
                          ...
                          <Grid>
                            <FormControl style={{ fontSize: '15px !important' }}>
                              ...
                              >
                                <FormControlLabel value="FAST" name='item_speed' onChange={(e) => { handleItemSpeed(e, index) }} checked={item.item_speed === "FAST"} control={<Radio />} label={data.item_speed_fast} style={{ fontSize: '15px !important' }} />
                                <FormControlLabel value="NORMAL" name='item_speed' onChange={(e) => { handleItemSpeed(e, index) }} checked={item.item_speed === "NORMAL"} control={<Radio />} label={data.item_speed_normal} style={{ fontSize: '15px !important' }} />
                                <FormControlLabel value="SLOW" name='item_speed' onChange={(e) => { handleItemSpeed(e, index) }} checked={item.item_speed === "SLOW"} control={<Radio />} label={data.item_speed_slow} style={{ fontSize: '15px !important' }} />
                              </RadioGroup>
                            </FormControl>
                          </Grid>
                        </Grid>
                          ...
                          <AccordionDetails>
                            ...
                            <Grid style={{ textAlign: 'center' }}>
                              ...
                              <Grid style={{ margin: '0 10px', display: 'flex', justifyContent: 'space-between', marginTop: 8, padding: '35px 0' }} mt={5} item xs={12} sm={12} md={12}>
                                <Grid style={{ paddingTop: 40 }} md={2}>
                                  {data.upload_video_title}
                                </Grid>
                                <Grid style={{ paddingTop: 40 }} md={4}>
        ...
        <>
          ...
          <Button style={{ float: 'right' }} className='subpro' onClick={updateProj}> {data.update_butt}</Button>
